...
class AStar(object):
    ...
    def insertion_sort(self, arr, simulation=False):
    ...
    def aStar(self):
            ...
            for i in range(len(self.straightLineDistances)):
                for j in range(len(neighbours)):
                    if self.straightLineDistances[i][0] == neighbours[j]:
                        neighbourDistances[neighbourDistancesCount][0] = self.straightLineDistances[i][0]
                        for k in range(0, len(self.distancesBetweenCities)):
                            if (self.distancesBetweenCities[k][0] == neighbours[j] and self.distancesBetweenCities[k][1] == self.startingCity) \
                                    or (self.distancesBetweenCities[k][1] == neighbours[j] and self.distancesBetweenCities[k][0] == self.startingCity):
                                    distanceBetweenTwoCities = self.distancesBetweenCities[k][2]
                        neighbourDistances[neighbourDistancesCount][1] = self.straightLineDistances[i][1]+distanceBetweenTwoCities
        ...
        solutionPath = []
        if solutionNode.name == "No Solution":
            solutionPath.append(solutionNode.name)
        else:
            for s in solution:
